...
## 7. Amendment 3 (3 Sep, after the first `/narrative` consumer run)

**Trigger.** Feedback 01788396626574918910 from a `/narrative` run: 52 grok keep-alive forks (chain tails whose only real prompt is the seat's scheduled loop prompt) were listed as unprocessed and would each have cost a delta-extractor call. Judge finding F1 (verdict 01788393451001602134) had already flagged 527 ingested user turns that are pure telemetry (`subagent_completed`, `task_completed`).

**Census (live store, 3 Sep 12:1x, 625 chain tails).** Kept user turns per tail after dropping telemetry turns: 1 turn 516, 2 turns 14, 3–9 turns 34, ≥10 turns 60, 0 turns 1. Only 2 tails are entirely synthetic, because a fork carries its parent's first real prompt — so an "all-synthetic" rule would not catch keep-alive forks; a threshold does. Tails passing a threshold after the telemetry drop: ≥1 → 624, ≥2 → 108, ≥3 → 94, ≥5 → 77.

**Decisions.**
- **A3.1** `adapters/grok.py` drops user records whose `synthetic_reason` is `subagent_completed` or `task_completed` even when `prompt_index` is present (D5 amended). `scheduler_fired` (the seat's task) and `notification_drain` (peer board prose) stay kept. Fixture 09's pinned expectations change accordingly (32 → 25 retained turns) and are regenerated by the test author.
- **A3.2** `_MIN_USER_TURNS_BY_CLIENT["grok"]` becomes 3 (D7 amended): seat runs are 5–10 prompts and interactive sessions longer; one- and two-prompt tails are keep-alive forks. The two exact-map assertions in `tests/test_narrative_coverage.py` change to `grok: 3` / `grok: 5`.
