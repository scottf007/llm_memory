...
Claim recall was manually matched by meaning, not string overlap; the paired run is directional evidence, not a release benchmark. Today's 70% local/Claude ratio confirms the 22 Aug bake-off's roughly 70% local recall. Today's sample had 0% `load_bearing` from both, so it does not erase the earlier 44% local versus 12% reference over-grading case. The scratch report's `reference_sonnet` summary is known-bad (it says 4 learnings / 5 done / 1 archive; the raw Claude delta has 2 / 6 / 0), so the qualification harness must count the validated raw deltas directly and must not reuse that summary code.
...
The worker takes a nonblocking host singleton flock, then `$LLM_MEMORY_HOME/runtime/locks/narrative/{project}.lock` (canonical project key) for the entire coverage → extract → validate → merge → render transaction. A losing trigger exits successfully because its unique request remains and the winner rechecks requests plus authoritative coverage before releasing the lock. Projects are alphabetical; sessions use `unprocessed_sorted`, then stale reruns. First slice is serial globally; cross-project parallelism waits.

That project lock is the single gate for automatic and manual work. The installed `skills/narrative/SKILL.md` must acquire it before reading condensed state and hold it through every ordered extract/merge/render; `merger.main` and `renderer.main` must acquire it by default for direct CLI use, while the worker/skill pass an inherited, verified lock handle to avoid reacquiring their own lock. If the worker owns it, a losing manual `/narrative` or hand-run CLI performs no extraction or mutation, exits non-zero, and prints `LLM_MEMORY_WARN: narrative update already running for {project}; retry after it completes`; if a manual drain owns it, the worker yields and leaves its request pending for the winner/timer recheck.
...
Automatic runs do **not** apply `ledger_delta.revaluations` while `lrn-6cc16ac2` is unresolved. Before validation or filtering, atomically preserve the exact extractor response in immutable `$LLM_MEMORY_HOME/runtime/extraction-results/{project}/{session_id}/{request_id}.json`; derive the applied delta from a parsed copy with `ledger_delta.revaluations` removed. The extraction record and project status expose `quarantined_revaluations` count plus that artifact path. Quarantined artifacts remain until explicit manual acknowledgement and for at least 30 days afterward; ordinary successful attempts retain 30 days. `contested.json` remains only the renderer's current token-budget cut and is not the preservation store. Automatic runs also do not run the irreversible cascade resolver. Out of scope: changing the delta-extractor prompt, solving commutative revaluation, changing cascade semantics, cross-project parallelism, multi-machine ownership/sync, or tuning the local model.
...
| Mutation | Keep `merger.main`/`apply_delta` as the only ledger mutation path; add the shared project-lock helper, default CLI locking, inherited-lock verification, raw-result quarantine, and provenance plumbing. Snapshot has one production path into `apply_delta` (`merger.main`) and one into `tools.project_state.write_full` (`merger.main`), plus their tests. |
...
| Seat | Produces |
|---|---|
| tests (Claude/Sonnet) | Frozen fake-home tests for enqueue/control, systemd install, duplicate triggers, re-trigger-during-drain, worker/manual lock contention, ordered two-session state handoff, stale rerun, backend qualification/fallback, provenance, warning/footer persistence, and raw-result revaluation/cascade quarantine. Trigger and non-trigger controls for every warning/guard. |
...
2. Two simultaneous triggers, a request arriving during drain, and worker-versus-`/narrative`/direct-CLI contention produce one serialized project history: every session once, extractor N+1 sees merge N, final coverage zero, active/archive split consistent, no concurrent index rebuild; a losing manual call prints the specified warning without reading state or mutating files.
...
4. Invalid local/Claude output and auth/download/budget failures never merge. Local is bypassed while unqualified. Successful state and reruns contain complete provenance; secrets and full prompts never enter status/logs. A revaluation-bearing response is preserved byte-for-byte at the stated per-session path, its applied copy has no revaluations, status names its count/path, and retention cannot remove it before acknowledgement plus 30 days.
