...
EOF
    ...
    echo "  session end. Requests for sessions already in sessions[] are"
    echo "  retired WITHOUT calling the backend, so a queue full of work a"
    echo "  manual /narrative already merged costs nothing to clear."
    echo "  Size the real exposure by counting that directory and subtracting"
    echo "  those; the request_ids in *.extraction-status.json are a stale"
